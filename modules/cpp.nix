...
with builtins;
{
  ...
  perSystem = { lib, config, pkgs, self', inputs', system, ... }:
    let
      ...
      clang'' = pkgs.pkgsMusl.llvmPackages_18.libcxxStdenv.cc;
      stdenv''' =
        pkgs.stdenvAdapters.overrideCC stdenv'' clang'';
        ...
        (final: prev:
          if !prev.stdenv.hostPlatform.isLinux then
            ...
            libedit = prev.libedit.overrideAttrs (old: {
              ...
              env.NIX_CFLAGS_COMPILE = "-D__STDC_ISO_10646__=201103L";
            });
            ...
            libxcrypt = prev.libxcrypt.overrideAttrs (old: {
              env.NIX_LDFLAGS = "${getBuildEnv old} --undefined-version";

              #old.NIX_FLAGS ++ final.lib.optional (prev.stdenv.cc.isClang)
            });

            ncurses = prev.ncurses.overrideAttrs
              (old: { env.NIX_LDFLAGS = "${getBuildEnv old} --undefined-version"; });
            ...
            binutils-unwrapped = (prev.binutils-unwrapped).overrideAttrs
              (old: {
                #env.NIX_LDFLAGS = "${getBuildEnv old} --undefined-version";
                configureFlags = old.configureFlags ++ [ "LDFLAGS=-Wl,--undefined-version" "CFLAGS=-Wl,--undefined-version" ];
          ...
          import self.inputs.nixpkgs
            {
              ...
              # linker = "lld";
              crossSystem = self.inputs.nixpkgs.lib.systems.examples.musl64 // {
                useLLVM = true;
              };
            }
        else
          import self.inputs.nixpkgs { inherit system overlays; };
    ...
    in
    {
      devShells = {
        ...
        tc = (pkgs.mkShell.override { stdenv = stdenv'''; }) {
          nativeBuildInputs = [
            pkgs.cmake
            pkgs.ninja
          ];
